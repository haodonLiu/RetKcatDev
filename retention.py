...
class SimpleRetention(nn.Module):
    def __init__(self, hidden_size, gamma, head_size=None):
        """
...
class MultiScaleRetention(nn.Module):
    def __init__(self, hidden_size, heads):
        ...
        self.head_v_dim = hidden_size 
        
        self.gammas = (1 - torch.exp(torch.linspace(math.log(1/32), math.log(1/512), heads))).detach().tolist()
        ...
        self.retentions = nn.ModuleList([
            SimpleRetention(self.hidden_size, gamma, self.head_size) for gamma in self.gammas
        ])
